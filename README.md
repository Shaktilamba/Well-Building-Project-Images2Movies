...
* A ```subsampling_rate``` video: 
  * These videos are created using a single fixed rate of subsampling (so that that all they look like they're the same speed) and have fixed frame rates of 50 fps. They are all 5 minutes long. However, the videos will have varying durations due to the different number of images which need to go into making individual videos. For instance, if we subsample 10% of images from 100 and 1000 images, we'll end up with videos containing 10 and 100 frames. Both will have been subsampled at the same rate, but the longer image sequence will still make a longer video. To be clear, images are omitted using this technique. 
  * Why would you choose this? Let's say you have a specific image sequence (SIS) which works well as a 5 minute video. Let's say that this SIS is very long, and cannot be converted to a 5 minute video using a frame rate between 1 and 50. Thus, this SIS would be subsampled at a specific rate, one calculated by the code.   
